...
const updateUserAvatar = asyncHandler(async (req, res) => {
  try {
    ...
    if (!newAvatarLocalPath)
      throw new ApiError(404, "New Avatar File is required");
...
const updateFullName = asyncHandler(async (req, res) => {
  try {
    ...
    if (!newName || !password)
      throw new ApiError(400, "Name & password field is required");
    const user = await User.findById(userID);
    if (!user) throw new ApiError(404, "Cannot find User");
    const isMatch = await user.isPasswordCorrect(password);
    if (!isMatch) throw new ApiError(400, "Invalid Old Password");
    const userWithUpdatedName = await User.findByIdAndUpdate(
      userID,
      { fullName: newName },
      { new: true }
...
const getUserWatchHistory = asyncHandler(async (req, res) => {
  try {
    ...
    if (
      !returnArray ||
      !returnArray.length ||
      !returnArray[0].watchHistory.length
    ) {
      return res
        .status(200)
